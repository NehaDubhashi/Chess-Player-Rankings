...
public class BinaryMaxHeap <T extends Comparable<T>> implements MyPriorityQueue<T> {    
    ...
    private void percolateDown(int i){
        ...
        while(left < size){  // until location is leaf
            int toSwap = right;
            if(right >= size || (arr[left].compareTo(arr[right]) > 0 )){  // if there is no right child or if left child is greater
                toSwap = left;  // swap with left
            } // now toSwap has the greater of left/right, or left if right does not exist
            if (arr[toSwap].compareTo(val) > 0) {  // if the greater child is greater than the current value
                arr[i] = arr[toSwap];
                arr[toSwap] = val; // swap parent with greater child
                i = toSwap; // update current node to be greater child
                left = i*2 + 1;
                right = i*2 + 2; 
                for (int j = 0; j < size; j++) {
                    itemToIndex.put(arr[j], j); // Update map with new indices
    ...
    private T remove(int index){
        if (size == 0) { 
            ...
            throw new IllegalArgumentException("Invalid heap input RM.");
        }
        T temp;
    ...
    private void updatePriority(int index){
        ...
        if (index == 0) {
            ...
            if (arr[index].compareTo(arr[parent]) > 0) {
                // If item is greater than its parent, percolate up
                percolateUp(index);
            } else {
